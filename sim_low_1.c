...
{
	//Printout to file
	FILE* nhopFile = fopen ("nhopFile_low_1.tr", "a+");
	FILE* collisionFile = fopen("collisionFile_low_1.tr", "a+");
	FILE* propDelayFile = fopen("nslotFile_low_1.tr","a+");
				...
				for (msg = 0; msg < n_msg; msg++){
					...
					if (p[msg].is_sent){
						...
						for (index = 0; index < n; index++) {
							...
							if (node_dist <= t_range && node_dist > 0){
